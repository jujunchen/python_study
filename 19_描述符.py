...
1.数据描述符（Data Descriptor）：实现了__get__和__set__方法的数据描述符。数据描述符会覆盖实例字典中同名的属性
2.非数据描述符（Non - data Descriptor）：只实现了__get__方法的描述符。非数据描述符不会覆盖实例字典中同名的属性，当实例字典中有同名属性时，会优先使用实例字典中的值
3.可调用描述符（Callable Descriptor）：实现了__call__方法的描述符，使描述符对象可以像函数一样被调用
